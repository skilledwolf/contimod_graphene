...
| Koshino & McCann, Phys. Rev. B 81, 115315 (2010) | Inversion-protected valley degeneracy in even Bernal stacks and all rhombohedral stacks | Anchors the LL `K/K'` spectral-degeneracy checks that are currently asserted for Bernal bilayer/tetralayer at `U=Delta=0` and rhombohedral trilayer/tetralayer at `U=0` |
...
10. Inversion-protected LL valley degeneracy for Bernal bilayer/tetralayer and rhombohedral trilayer/tetralayer, compared through sorted `K/K'` spectra rather than raw LL matrices. The Bernal even-layer checks explicitly use `U=0` and `Delta=0`.
