...
- The slow ABA regression tier now includes the source-aligned `gamma3 = 0` bilayerlike low-pair check endorsed by the literature review: in the mirror-basis even block, the `n = -1` anchor follows Eq. (30) and the `n = 0 - (-1)` splitting follows Eq. (23) for the source-aligned `Delta = 0` convention, with explicit `n_cut` stability. The package-to-paper valley map used there is now source-backed (`K -> xi = -1`, `K' -> xi = +1`).
- ABA trilayer detailed finite-`gamma3` low-pair regressions remain deferred. The fast suite now covers exact mirror-basis decoupling at realistic ABA parameters, the exact monolayerlike odd-block content, and the exact source-aligned bilayerlike even-block `H_2` construction, but the literature does not provide a finite-`gamma3` observable for the bilayerlike `n = -1, 0` pair itself. Once `gamma3` is restored, those levels survive only as adiabatic descendants inside the full even block, so overlap/continuation tracking is treated as a package-level diagnostic idea rather than part of the paper-backed validation layer.
