...
- The exact rhombohedral fast tier now reaches generic higher-`N` LL structure through pentalayer `K/K'` valley degeneracy and clean-subset LL zero-mode counting. The next optional ABC addition would be a slower LL field-scaling fit `E ~ B^(N/2)` in the nearest-neighbor subset, not more low-field warped-LL structure.
